...
public class TestDataSet {
	public static void main(String[] args) throws Exception {
		// number of channels (the images are grayscale so there is only one
		// channel)
		int numChannels = 1;
		...
		int batchSize = 64;
		// how often to pass the entire dataset through the network during
		// training
		int numEpochs = 1;
		...
		DataSetIterator trainingSet = new FaultDataSetIterator(batchSize, 100);
		// fetch the MNIST testing dataset
		DataSetIterator testSet = new FaultDataSetIterator(batchSize, 20);
		...
		MultiLayerConfiguration configuration = new NeuralNetConfiguration.Builder().seed(seed)
				...
				.updater(new Nesterovs(0.1, 0.01)).list()
				// the first layer is a convolution layer with a kernel size of
				// 5x5 pixels
				.layer(0,
						new ConvolutionLayer.Builder(5, 5)
								// the number of channels are specified with the
								// nIn method
								.nIn(numChannels)
								// in each step move the kernel by just one
								// pixel
								.stride(1, 1)
								// number of kernels to use
								.nOut(20)
								// use the RELU-Function as an activation
								// function
				...
				.layer(2,
						new ConvolutionLayer.Builder(5, 5).stride(1, 1)
								...
								.nOut(50).activation(new ActivationReLU()).build())
				// use one more subsampling layer before the densely connected
				// network starts
				.layer(3,
						new SubsamplingLayer.Builder(SubsamplingLayer.PoolingType.MAX).kernelSize(2, 2).stride(2, 2)
								.build())
				...
				.layer(5,
						new OutputLayer.Builder(new LossNegativeLogLikelihood())
								...
								// classes
								.nOut(numClasses)
								// use the softmax function in the last layer so
								// the outputs can be interpreted as
								// probabilities
								.activation(new ActivationSoftmax()).build())
				// in the MNIST dataset, the 28x28 grayscale images are
				// represented as vectors, thus the input type is
				// convolutionalFlat
